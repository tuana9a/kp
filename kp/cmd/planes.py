...
class InitCmd(Cmd):
    ...
    def run(self):
        ...
        lb_ifconfig0 = PveService.current_config(api, node, lb_id).ifconfig(0)
        if not lb_ifconfig0:
...
class CreateDadControlPlaneCmd(Cmd):
    ...
    def run(self):
        ...
        PveService.update_config(api, node, dad_id,
                                 name=new_vm_name,
                                 cpu="cputype=host",
                                 cores=vm_cores,
                                 memory=vm_mem,
        ...
        PveService.resize_disk(api, node, dad_id, "scsi0", vm_disk_size)

        PveService.startup(api, node, dad_id)
        PveService.wait_for_guestagent(api, node, dad_id)
        PveService.wait_for_cloudinit(api, node, dad_id)

        userdata_location = "/usr/local/bin/userdata.sh"
        with open(vm_userdata, "r") as f:
            PveService.write_file(api, node, dad_id, userdata_location, f.read())
        PveService.exec(api, node, dad_id, f"chmod +x {userdata_location}")
        PveService.exec(api, node, dad_id, userdata_location)
